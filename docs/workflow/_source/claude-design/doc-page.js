...
(() => {
  ...
  class DocPage extends HTMLElement {
    ...
    _measure() {
      ...
      // First-paint guard: slotted header/footer may not have layout yet when
      // the initial measure runs (streaming mount) — re-measure until it does,
      // and again on window load, so --doc-hdr-h never sticks at 0.
      if (!this._settled) {
        const h = hdr ? hdr.offsetHeight : 0;
        const f = ftr ? ftr.offsetHeight : 0;
        if ((hdr && !h) || (ftr && !f) || (!hdr && !ftr)) {
          if ((this._retries = (this._retries || 0) + 1) < 40) {
            setTimeout(() => this._scheduleMeasure(), 50);
        ...
        if (!this._loadHooked) {
          ...
          window.addEventListener('load', () => this._scheduleMeasure(), { once: true });
        }
      }
    }
  }
